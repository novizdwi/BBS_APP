...
{
    public class ListFindParamReactiveTag
    {
    ...
    public class ReactiveTagView___
    {
    ...
    public class ReactiveTag__List_Model
    {
        static string ViewSql = "SELECT T0.*" +
        "FROM \"Tx_ReactiveTag\" T0 " +
        "ORDER BY T0.\"CreatedDate\" DESC";

        public static void SetBindingData(GridViewModel state, int userId, ListFindParamReactiveTag cflParam)
        {
        ...
        public static void GetData(GridViewCustomBindingGetDataArgs e, List<ReactiveTagView___> dataList)
        {
            e.Data = dataList;
        }

        public static int GetRowCount(HANA_APP CONTEXT, int userId, ListFindParamReactiveTag param, string sqlCriteria)
        {
        ...
        public static List<ReactiveTagView___> GetDataList(HANA_APP CONTEXT, int userId, ListFindParamReactiveTag param, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            var views = new List<ReactiveTagView___>();
            ...
            if (param != null)
            {
                if (param.IsFindTransDate)
                {
                    ...
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 AND \"TransDate\"<=:p1 ";
                        views = CONTEXT.Database.SqlQuery<ReactiveTagView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date, param.TransDate_To.Value.Date).ToList();
                    }
                    else if (param.TransDate_From != null)
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 ";
                        views = CONTEXT.Database.SqlQuery<ReactiveTagView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date).ToList();
                    }
                    else if (param.TransDate_To != null)
                    {
                        //ssql = ssql + " AND \"TransDate\"<=:p0 ";
                        views = CONTEXT.Database.SqlQuery<ReactiveTagView___>(ssql + sqlSort + ssqlLimit, param.TransDate_To.Value.Date).ToList();
                    }
                }
                else
                {
                    views = CONTEXT.Database.SqlQuery<ReactiveTagView___>(ssql + sqlSort + ssqlLimit).ToList();
                }
            }
            else
            {
            ...
            if (views.Count == 0)
            {
                ReactiveTagView___ view = new ReactiveTagView___();
                views.Add(view);
            }
