...
namespace Models.Transaction.TukarFakturSend
{
    ...
    public class TukarFakturSend__List_Model
    {
        static string ViewSql = "SELECT T0.* FROM \"Tx_TukarFakturSend\" T0 ";
        ...
        public static int GetRowCount(HANA_APP CONTEXT, int userId, ListFindParam param, string sqlCriteria)
        {
            ...
            if (param != null)
            {
                if (param.IsFindTransDate)
                {
                    ...
                    {
                        ssql = ssql + " AND \"TglDokumen\">=:p0 AND \"TglDokumen\"<=:p1 ";
        ...
        public static List<View___> GetDataList(HANA_APP CONTEXT, int userId, ListFindParam param, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            if (sqlSort == "")
            {
                sqlSort = " ORDER BY \"TglDokumen\" DESC ";
            }
        ...
        public static GridViewSettings CreateExportGridViewSettings()
        {
            ...
            settings.Name = "List Tukar Faktur (Send)";

            settings.KeyFieldName = "Id";
            settings.Columns.Add("Id").Visible = false;
            settings.Columns.Add("No Dokumen", "No Dokumen");
            settings.Columns.Add("TglDokumen", "Tgl Dokumen", MVCxGridViewColumnType.DateEdit).PropertiesEdit.DisplayFormatString = "dd/MM/yyyy";
