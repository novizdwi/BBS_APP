...
namespace Models.Transaction.TukarFakturReceipt
{
    ...
    public class TukarFakturReceipt__List_Model
    {
        static string ViewSql = "SELECT T0.* FROM \"Tx_TukarFakturReceipt\" T0 ";
        ...
        public static int GetRowCount(HANA_APP CONTEXT, int userId, ListFindParam param, string sqlCriteria)
        {
            ...
            if (param != null)
            {
                if (param.IsFindTransDate)
                {
                    ...
                    {
                        ssql = ssql + " AND \"TglTerima\">=:p0 AND \"TglTerima\"<=:p1 ";
        ...
        public static List<View___> GetDataList(HANA_APP CONTEXT, int userId, ListFindParam param, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            if (sqlSort == "")
            {
                sqlSort = " ORDER BY \"TglTerima\" DESC ";
            }
        ...
        public static GridViewSettings CreateExportGridViewSettings()
        {
            ...
            settings.Name = "List Tukar Faktur (Receipt)";

            settings.KeyFieldName = "Id";
            settings.Columns.Add("Id").Visible = false;
            settings.Columns.Add("NoTglTerima", "No Tgl Terima");
            settings.Columns.Add("TransDate", "Tgl Terima", MVCxGridViewColumnType.DateEdit).PropertiesEdit.DisplayFormatString = "dd/MM/yyyy";
