...
namespace Controllers.Master.Item
{
    ...
    {
        public ListFindParamReactiveTag GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamReactiveTag();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvReactiveTagList");
            if (viewModel == null)
            {
                viewModel = ReactiveTag__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamReactiveTag param)
        {

            ReactiveTag__List_Model.SetBindingData(viewModel, userId, param);
