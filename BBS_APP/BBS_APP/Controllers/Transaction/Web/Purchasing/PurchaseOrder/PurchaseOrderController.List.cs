...
namespace Controllers.Transaction.Web.Purchasing
{
    ...
    {
        public ListFindParamPurchaseOrder GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamPurchaseOrder();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvPurchaseOrderList");
            if (viewModel == null)
            {
                viewModel = PurchaseOrder__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamPurchaseOrder param)
        {

            PurchaseOrder__List_Model.SetBindingData(viewModel, userId, param);
